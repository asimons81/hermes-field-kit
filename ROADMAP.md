# Roadmap

This roadmap describes direction, not a promise of dates.

## Foundation

- [x] Define the admission rule
- [x] Establish governance and contribution policy
- [x] Define the Hermes-compatible skill contract
- [x] Align the repository layout with Hermes tap discovery
- [x] Add catalog and test-case schemas
- [x] Add dependency-free validation
- [x] Add validator rejection tests
- [x] Add hardened continuous integration
- [ ] Enable and verify the `main` repository ruleset and native security controls

## First release

- [x] Select one proven daily-driver skill
- [x] Remove personal data and environment-specific assumptions
- [x] Add realistic examples and behavior tests
- [x] Validate repository-qualified installation in a fresh Hermes environment
- [ ] Validate custom tap-backed search when Hermes exposes that path reliably
- [x] Add the first catalog entry
- [x] Cut the first tagged release

## Later

- [ ] Add compatibility reporting across Hermes versions
- [ ] Add catalog generation from validated skill metadata
- [ ] Add packaged release artifacts when justified
- [ ] Document a deprecation process based on real maintenance needs

The repository will not set a skill-count target. Volume is not a success metric.
